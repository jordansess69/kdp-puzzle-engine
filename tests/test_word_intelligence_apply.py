...
@pytest.fixture
def decided_store(tmp_path):
    """Store with one human-approved and one human-rejected decision.

    The decision log MUST live under pytest's tmp_path - writing it into
    tests/ polluted the working tree with an artifact file.
    """
...
def test_collect_decisions_requires_human_evidence_for_rejections(decided_store):
    store, _ = decided_store
...
def test_plan_reports_counts_without_writing(decided_store):
    store, _ = decided_store
...
def test_dry_run_is_default_and_writes_nothing(tmp_path, decided_store):
    store, _ = decided_store
...
def test_apply_writes_validated_curated_source(tmp_path, decided_store):
    store, _ = decided_store
...
def test_payload_carries_topic_raw_names_for_builder(tmp_path, decided_store):
    store, taxonomy = decided_store
    state_dir = tmp_path / "state"
    state_dir.mkdir(parents=True)
